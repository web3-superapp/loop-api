(function(){
  ...
  function perpFixture(){return seal({kind:'perp_order',execution:{provider_path:'hyperliquid',wallet_id:WALLET,payload:{request_id:'review-perp',endpoint:'hyperliquid:testnet',method:'order',chain_id:'hyperliquid-testnet',market:'ETH',side:'buy',order_type:'market',size:'0.01',leverage:'3',reduce_only:false,environment:'testnet'},execution_digest:''},context:common('hyperliquid_order_fixture',[TOKENS.ETH],null,null,{spender:null,provider:'Hyperliquid',environment:'testnet'}),source_digest:'',expires_at_ms:500000});}
  ...
  function origin(value){const item=record(value,['stack','voice'],[],'origin'),stack=array(item.stack,26,'stack'),voice=record(item.voice,['state','open','minimized','muted'],[],'voice'),excluded=['scr-notifications','scr-search','scr-privacy','scr-security'];if(!stack.length||stack.some(entry=>typeof entry!=='string'||!/^scr-[a-z0-9]+(?:-[a-z0-9]+)*$/.test(entry)||excluded.includes(entry))||!['idle','joining','joined','reconnecting','error'].includes(voice.state)||['open','minimized','muted'].some(key=>typeof voice[key]!=='boolean'))throw new TypeError('origin');return freeze({stack:[...stack],voice:{...voice}});}
  ...
  function createController(options){
    const selected=record(options,['adapter'],[],'options'),adapter=captureAdapter(selected.adapter),sessions=new Map(),consumed=new Set(),forwardBlocked=new Set(),acknowledged=new Set(),returning=new Set(),handoffOutcomes=new Map();
    ...
    const unchanged=session=>{const repeated=decodeInternal(session.source);return same(repeated.model,session.model)&&same(repeated.execution_binding,session.execution);};
