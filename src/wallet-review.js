(function(){
  ...
  function origin(value){const item=record(value,['stack','voice'],[],'origin'),stack=array(item.stack,26,'stack'),voice=record(item.voice,['state','open','minimized','muted'],[],'voice');if(!stack.length||stack.some(entry=>typeof entry!=='string'||!/^scr-[a-z0-9]+(?:-[a-z0-9]+)*$/.test(entry))||!['idle','joining','joined','reconnecting','error'].includes(voice.state)||['open','minimized','muted'].some(key=>typeof voice[key]!=='boolean'))throw new TypeError('origin');return freeze({stack:[...stack],voice:{...voice}});}
