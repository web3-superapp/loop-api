...
import {
  ...
  type CommunityChannelPersonaBackfillTarget,
      ...
      text: `
        ...
        set
          ...
          next_projection_at = clock_timestamp()
            + make_interval(secs => $${String(values.length + 1)}::double precision),
          updated_at = clock_timestamp()
        where ${where}
      `,
      values: [...values, delay],
  ...
  return Object.freeze({
    async ensurePersona(input: {
      ...
      try {
        const communityId = opaqueIdSchema.parse(input.communityId);
        const ownerUserId = userIdSchema.parse(input.ownerUserId);
        const existing = await findPersona(communityId, ownerUserId);
        if (existing !== null) {
          return existing;
        }
        for (
          ...
          try {
            const inserted = await pool.query<Record<string, unknown>>({
              text: `
                ...
                select ${personaColumns} from inserted as persona
              `,
              values: [communityId, ownerUserId, alias],
            });
            const row = inserted.rows[0];
            if (row !== undefined) {
            ...
            const raced = await findPersona(communityId, ownerUserId);
            if (raced !== null) {
              return raced;
            }
            throw new CommunicationRepositoryUnavailableError();
    ...
    async confirmProjection(input: {
      ...
      try {
        await pool.query({
          ...
          `,
          values: [opaqueIdSchema.parse(input.personaId)],
        });
      } catch (error) {
        return translateRepositoryError(error);
      }
    ...
    async resetProjectionForMember(input: {
      ...
      try {
        await resetProjection(
          "persona.community_id = $1 and persona.owner_user_id = $2",
          [
            opaqueIdSchema.parse(input.communityId),
            userIdSchema.parse(input.ownerUserId),
          ],
    ...
    async claimPendingProjections(input: {
      ...
      try {
        ...
        return Object.freeze(
          result.rows.map((row) => {
            const persona = mapPersonaRow(row);
            return Object.freeze({
              persona,
              streamChannelId: streamCommunityChannelIdSchema.parse(
                row["stream_channel_id"],
              ),
              memberStreamUserId: deriveStreamUserId(persona.ownerUserId),
            });
          }),
        );
      } catch (error) {
    ...
    async listBackfillTargets(input: {
      readonly limit: number;
      readonly after: {
        readonly communityId: string;
        readonly ownerUserId: string;
        ...
        const result = await pool.query<Record<string, unknown>>({
          text: `
            ...
            join public.community_channels as channel
              ...
              and channel.state <> 'failed'
            left join public.community_channel_personas as persona
              on persona.community_id = member.community_id
