...
| `MARKET_PROVIDER_RESPONSE_MALFORMED`     | Provider 响应不符合契约（含 JSON 数字精度丢失）。注意：单个"池标识不是地址"的交易对（four.meme `{address}:4meme`、Uniswap V4 pool id）只会被丢弃并计数，不再让整个 token 的行情事实变成不可用（决策 0060） |
