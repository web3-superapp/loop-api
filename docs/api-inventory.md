...
Registered with the `community` module, but gated on one additional fact: an
`ANTHROPIC_API_KEY`. Without it the `communityAi` capability stays `deferred`
