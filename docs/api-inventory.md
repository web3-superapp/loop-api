...
| `GET /v2/communities/{communityId}`                        | Bearer + contract/client headers; no payload                                   | Community record, viewer membership and permission flags, `miningPower`                | `implemented` | `implemented`; `miningPower` from the latest snapshot under the formula in force (Decision 0043), tagged `subject: community` with the version `scope` plus the same `weight`/`participants` as `GET /v2/mining/communities/{id}` (Decision 0045); presence/announcements/links `unavailable`                                                                                                                                                    |
...
| `GET /v2/mining/rank`                      | `scope=users\|communities` | Positive-power ranking (≤100), `myPosition`, alias-or-anonymous rule; `formula` as on the summary (Decision 0046)                                                                                             | `implemented` | same as summary                                                                                         |
