...
| `GET /v2/mining/assets`                    | Bearer + headers           | `included` power rows + `excluded` held assets with the reason the latest attempt recorded (`MINING_PRICE_PAIR_NOT_FOUND`, …) or re-derived, each with the registry `symbol`; `source.stale` + `latestAttempt`; `formula` as on the summary                                                          | `implemented` | same; `503 CAPABILITY_UNAVAILABLE` when the Asset Registry cannot answer                                |
