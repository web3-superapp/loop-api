...
| Method and path                            | Request                    | Success projection                                                                                                                                                                                            | Interface     | Capability                                                                                              |
| ------------------------------------------ | -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- | ------------------------------------------------------------------------------------------------------- |
| `GET /v2/mining/summary`                   | Bearer + headers           | `power`, `networkPower`, `estimatedToday` (budget × share) from the latest snapshot under the version in force; `referralBoost` `MINING_REFERRAL_BOOST_PENDING` (Decision 0046)                               | `implemented` | `implemented` under an approved version (Development baseline); product numbers `blocked-product-legal` |
| `GET /v2/mining/assets`                    | Bearer + headers           | `included` power rows + `excluded` held assets with the lane's skip reason, each with the registry `symbol`; `formula` as on the summary (Decision 0046)                                                      | `implemented` | same; `503 CAPABILITY_UNAVAILABLE` when the Asset Registry cannot answer                                |
...
| `GET /v2/mining/rank`                      | `scope=users\|communities` | Positive-power ranking (≤100), `myPosition`; name by anonymous mode only, `power` null for others when `mining_power_visibility=self` (Decision 0049); `formula` as on the summary (Decision 0046)            | `implemented` | same as summary                                                                                         |
| `GET /v2/mining/communities/{communityId}` | Bearer + headers           | Weight record + members' power on the bound asset, caller's contribution, rank, participants; an unbound community's `weight` is `COMMUNITY_ASSET_NOT_BOUND` / `reviewStatus: not_applicable` (Decision 0046) | `implemented` | needs a bound asset and an approved weight (`pnpm mining:community-weight`)                             |
...
| `GET /v2/market/new-pairs`                | no input                                      | GeckoTerminal new pools when enabled; risk screening `unavailable`                                                                                                  | `implemented` | `explicitly-disabled` by default until GeckoTerminal terms are verified; enabled on the Development stack only (Decision 0050) |
...
| `POST /v2/wallet-intents/approve`                     | `Idempotency-Key`; `{walletId, assetId, spenderAddress, allowance, acknowledgeUnlimited?}`                 | Approve intent with decoded `approve(spender, value)`; the ceiling prices `min(allowance, balance)`                             | `implemented` | `blocked-product-legal`; same switch                                                                                                                                                                                                                                                |
...
`BSC_WRITE_CANARY_MAX_USD` on a fresh Provider price. Ambiguous Provider
...
already shows the matching payload. No transaction was broadcast or executed
locally.
...
| `GET /v2/communities/{communityId}/voice-rooms/current` | Bearer + contract/client headers                                              | The live room, or `null` with `COMMUNITY_VOICE_ROOM_NOT_LIVE`                                                      | `implemented` | `implemented`; `observed.participantCount` (live session) vs `memberCount` (authorized), own `observedAt` (0051) |
| `GET /v2/voice-rooms/{voiceRoomId}`                     | Bearer + contract/client headers                                              | Room incl. `communityName` (0052), viewer role, viewer hand raise, `joinedCount` incl. host                        | `implemented` | `implemented`; non-members are `PERMISSION_DENIED`                                                               |
...
| `POST /v2/voice-rooms/{voiceRoomId}/join`               | Write headers; no payload                                                     | Room resource with the caller's role, `expiresAt`, counts observed after the Stream write                          | `implemented` | `implemented`; idempotent; an unprovisioned room is `CAPABILITY_UNAVAILABLE`                                     |
| `POST /v2/voice-rooms/{voiceRoomId}/leave`              | Write headers; no payload                                                     | Room resource with `viewer.role: null`, counts observed after the Stream write                                     | `implemented` | `implemented`; the host must end the room instead                                                                |
...
| `GET /v2/voice-rooms/{voiceRoomId}/members`             | Bearer + contract/client headers; `role` required, `cursor`/`limit` exclusive | LOOP joined roster of one role view with leaderboard display rule, `handRaised`, `muted`, host `commands[]` (0052) | `implemented` | `implemented`; authorization roster, never Stream presence; anonymous rows unaddressable to non-hosts            |
