...
| `GET /v2/communities/{communityId}/members`                | `role=all\|owner\|admin\|banned`, `q` alias prefix, cursor page                | Owner→admin→member grouping, server counts, viewer permission flags          | `implemented` | `implemented`; `q` is a 1-40 code point alias prefix on the shared public alias search quota (Decision 0040); per-member mining power and online count `unavailable` |
