...
| `voiceRooms`    | The same availability, but `evidence` is always `{status: "pending", reasonCode: "AUDIO_ROOM_ROLE_EVIDENCE_PENDING"}` until the Decision 0005 Dashboard export exists                                                                                                               |
