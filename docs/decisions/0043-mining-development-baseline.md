# Decision 0043: Mining development baseline (`miningFormula-devBaseline-2026-09-15`)

- Status: Accepted (Development only); the product formula stays on the
  external Go/No-Go list until `02-合约产品方案` arrives
- Date: 2026-09-15
...
| Ranking                    | Accounts (or communities) with positive power, `rank()` with shared positions on ties, at most 100 rows. Alias only for a discoverable, non-anonymous, active profile; otherwise `mining.rank.anonymousMember`. A zero-power caller is `MINING_RANK_NOT_RANKED`; the community scope has no `myPosition` (`MINING_RANK_NOT_APPLICABLE`).                                                                                                                                                                                                                                                                                                                        |
...
| Capability                 | `communityMining` is decided per request by `resolveMiningBaseline`: `deferred(V2_MINING_RUNTIME_DEFERRED)` without the `mining` module, `unavailable(MINING_RUNTIME_UNAVAILABLE)` without the repository or on a read failure, `unavailable(MINING_FORMULA_BASELINE_PENDING)` without an approved, effective version, `available` otherwise. Its evidence stays `{pending, MINING_FORMULA_BASELINE_PENDING}` — the 02 freeze — in every state, as does the evidence of `mining` and `referral`. The Decision 0038 baseline fixture is regenerated for the deferred shape.                                                                                      |
...
| Price guard                | Unchanged from 0036: a fresh, non-proxied DexScreener price per asset. Native BNB's price is proxied through WBNB and is therefore skipped (`MINING_PRICE_NOT_FRESH`) — a policy question for the main agent, not changed here.                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
- `docs/frontend-v2-mining-api.md` §0 records the real Development run of
  2026-09-15: the formula row, the snapshot `3d0a9993-…` with six power rows
  at block `122037728`, and the projections read for account `3bb58597-…`.
