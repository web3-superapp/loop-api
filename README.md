...
- `src/screens-order.txt` 决定屏顺序，`src/scripts-order.txt` 决定脚本顺序，`src/vendor/vendor-lock.json` 锁定本地 QR 依赖来源；`_tmp/verify_wallet_foundation.py` 同时校验生成物、供应商边界、金额精度、历史投影、无障碍与安全扫描。
- **全项目未完成**：Stream 通信/语音、Hyperliquid Perp 与其余 A–I 供应商边界仍待实现（pending / in progress），继续遵循“集成优先”。
