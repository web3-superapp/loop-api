...
PRODUCTION_SCRIPTS=[
 ...
 "wallet-transfer.js","stream-chat-provider.js","app.js"]
...
def check_integrated_build(root:Path,provider:Path,fixture:Path)->None:
 ...
 if manifest!=PRODUCTION_SCRIPTS:raise VerificationError(f"exact six-script production order mismatch: {manifest}")
 ...
 if "exact pinned six-script order" not in build_source or "stream-chat-provider.js" not in build_source:raise VerificationError("builder six-script pin missing")
